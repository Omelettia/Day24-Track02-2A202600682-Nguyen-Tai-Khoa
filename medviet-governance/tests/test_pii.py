...
@pytest.fixture
def sample_df():
    # cccd/so_dien_thoai MUST be read as str, else pandas infers int and strips
    # leading zeros (phones lose their leading 0, 0-prefixed CCCDs drop to 11 digits).
    return pd.read_csv(
...
class TestAnonymization:
    ...
    def test_pii_not_in_output(self, anonymizer, sample_df):
        ...
        haystack = df_anon.astype(str).agg(" ".join, axis=1).str.cat(sep=" ")
        for original_cccd in sample_df["cccd"]:
            assert str(original_cccd) not in haystack
    ...
    def test_non_pii_columns_unchanged(self, anonymizer, sample_df):
        ...
        assert df_anon["patient_id"].tolist() == sample_df["patient_id"].tolist()

    def test_anonymize_text_replace_removes_cccd(self, anonymizer):
        """anonymize_text('replace') phải xóa CCCD gốc."""
        out = anonymizer.anonymize_text("CCCD: 012345678901", strategy="replace")
